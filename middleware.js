...
async function sessionValue(user, password) {
  ...
  const sig = await crypto.subtle.sign('HMAC', key, encoder.encode(`acf-v35:${user.toLowerCase()}`));
...
export default async function middleware(request) {
  ...
  const expectedSession = await sessionValue(expectedUser, expectedPassword);
  const authenticated = safeEqual(cookieValue(request, 'acf_session'), expectedSession);
  ...
  if (url.pathname === '/login') {
    if (request.method === 'POST') {
      ...
      if (safeEqual(user.toLowerCase(), expectedUser.toLowerCase()) && safeEqual(password, expectedPassword)) {
        return new Response(null, { status: 303, headers: {
          'Location': '/',
          'Set-Cookie': `acf_session=${expectedSession}; Path=/; HttpOnly; Secure; SameSite=Strict; Max-Age=2592000`,
