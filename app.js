...
function setupWarrantyRegistration(){
  ...
  const DB='acf-warranty-v1', STORE='records', THIRTY_DAYS=30*24*60*60*1000; let boilerBlob=null, filterBlob=null;
  ...
  const purgeOld=async rows=>{const now=Date.now();const old=rows.filter(r=>r.registeredAt && now-r.registeredAt>=THIRTY_DAYS);if(old.length) await Promise.all(old.map(r=>del(r.id)));return rows.filter(r=>!old.some(o=>o.id===r.id));};
  const card=(r,registered=false)=>`<div class="warranty-record ${registered?'registered-record':''}"><div class="warranty-record-head"><label class="warranty-check"><input type="checkbox" data-register-war="${r.id}" ${registered?'checked':''}><span></span></label><div class="warranty-address"><strong>${esc(r.address||'No address')}</strong><div class="meta-line">${esc(r.manufacturer)} ${r.date?'· '+esc(r.date):''}</div></div></div><div class="meta-line">Serial: <strong>${esc(r.serial||'—')}</strong></div>${registered?`<div class="meta-line">Registered ${new Date(r.registeredAt).toLocaleDateString('en-GB')} · disappears after 30 days</div>`:''}<div class="warranty-actions"><button data-copy-war="${r.id}">Copy serial</button><button data-delete-war="${r.id}">Delete</button></div></div>`;
  const bind=(box,rows)=>{box.querySelectorAll('[data-copy-war]').forEach(b=>b.onclick=async()=>{const r=rows.find(x=>x.id===b.dataset.copyWar);if(r?.serial){await navigator.clipboard.writeText(r.serial);b.textContent='Copied';setTimeout(()=>b.textContent='Copy serial',900);}});box.querySelectorAll('[data-delete-war]').forEach(b=>b.onclick=async()=>{if(confirm('Delete this saved warranty record?')){await del(b.dataset.deleteWar);render();}});box.querySelectorAll('[data-register-war]').forEach(c=>c.onchange=async()=>{const r=rows.find(x=>x.id===c.dataset.registerWar);if(!r)return;r.registeredAt=c.checked?Date.now():null;await put(r);render();});};
  const render=async()=>{let rows=await purgeOld(await all());const pending=rows.filter(r=>!r.registeredAt), registered=rows.filter(r=>r.registeredAt);const box=$('warrantyList'), arch=$('warrantyArchive');box.innerHTML=pending.length?pending.map(r=>card(r,false)).join(''):'<div class="meta">No warranties waiting to be registered.</div>';arch.innerHTML=registered.length?registered.map(r=>card(r,true)).join(''):'<div class="meta">No recently registered warranties.</div>';bind(box,rows);bind(arch,rows);};
  document.querySelectorAll('[data-warranty-tab]').forEach(b=>b.onclick=()=>{document.querySelectorAll('[data-warranty-tab]').forEach(x=>x.classList.toggle('active',x===b));document.querySelectorAll('.warranty-tab-panel').forEach(x=>x.classList.toggle('active',x.id===`warranty-tab-${b.dataset.warrantyTab}`));if(b.dataset.warrantyTab!=='new')render();});
