...
function setupRadiatorCalculator(){
  ...
  btn.onclick=()=>{
    ...
    const result=suitable.length ? suitable.map((r,i)=>`<div class="rad-option"><strong>${i===0?'Recommended: ':''}${r.type} · ${r.h} × ${r.l} mm</strong><br><small>${r.code} · ${r.w.toLocaleString()} W · ${r.btu.toLocaleString()} BTU/h · ${gbp(r.price)} + VAT</small></div>`).join('') : '<p>Requirement is above the largest single radiator in the catalogue. Use multiple radiators or split the load.</p>';
